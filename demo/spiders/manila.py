...
class ManilaSpider(scrapy.Spider):
        ...
        for p in range(int(allPages)):
            if flag:
                ...
                essaysList = BeautifulSoup(
                    json.loads(
                        r.text)['data'], 'html.parser').select('div.articlecontext')
                for i in essaysList:
                    pub_time = Util.format_time2(re.findall('\d+ \w+ ago',i.text)[0])
    ...
    #     essaysList = BeautifulSoup(
    #         json.loads(
    #             response.text)['data'], 'html.parser').select('div.articlecontext')
    #     for i in essaysList:
    #         pub_time = Util.format_time2(i.text)
    #         if self.time == None or Util.format_time3(pub_time) >= int(self.time):
    #             yield Request(i.select_one('a').get('href'), callback=self.parse_item, meta=response.meta)
    #         else:
    #             self.logger.info('时间截止')
