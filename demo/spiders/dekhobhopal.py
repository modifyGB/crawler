...
class dekhobhopalSpider(scrapy.Spider):
    name = 'dekhobhopal'
    ...
    def __init__(self, time=None, *args, **kwargs):
        super(dekhobhopalSpider, self).__init__(*args, **kwargs) # 将这行的DemoSpider改成本类的名称
        self.time = time
    ...
    def parse(self, response):
        ...
        category_hrefList = []
        # category_nameList = []
        categories = soup.select('ul#menu-td-demo-header-menu-1 li a') if soup.select('ul#menu-td-demo-header-menu-1 li a') else None
        del categories[0:2]
