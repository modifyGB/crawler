...
class pravakta(scrapy.Spider):
    ...
    def __init__(self, time=None, *args, **kwargs):
        super(pravakta, self).__init__(*args, **kwargs) # 将这行的DemoSpider改成本类的名称
        self.time = time
    ...
    def get_news_detail(self,response):
        ...
        image_list = soup.find("div", class_="entry-content").find("figure",class_="wp-block-image size-large").select_one("img").get("data-src") if soup.find("div", class_="entry-content").find("figure",class_="wp-block-image size-large") else None
        body = ''
