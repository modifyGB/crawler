...
def sinceindependence_time_switch1(time_string):
    # 返回时间戳
    #  जनवरी 3, 2021
    Hindi_month = {
...
def sinceindependence_time_switch2(time_string):
    # 返回 %Y-%m-%d %H:%M:%S
    ...
    return datetime.strptime(time_string, "%m-%d-%y %H:%M")

def sinceindependence_time_switch3():
    pass
...
class SinceindependenceSpider(scrapy.Spider):
    name = 'sinceindependence_spider'
    ...
    def __init__(self, time=None, *args, **kwargs):
        super(SinceindependenceSpider, self).__init__(*args, **kwargs)  # 将这行的DemoSpider改成本类的名称
        self.time = time
    ...
    def parse_category1(self, response):
        ...
        timestamp = sinceindependence_time_switch1(date)
        ...
        elif self.time and timestamp >= self.time and next_button:
            yield scrapy.Request(next_url, callback=self.parse_category1)
        else:
    ...
    def parse_detail(self, response):
        ...
        pub_time = soup.select_one(".jeg_meta_container .jeg_meta_date").text.strip()
        try:
            item['pub_time'] = sinceindependence_time_switch2(pub_time)
        except:
            item['pub_time'] = sinceindependence_time_switch3(pub_time)
        images = []
