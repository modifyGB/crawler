...
class WorldSpider(scrapy.Spider):
    ...
    def parse_menu(self, response):
        soup = bs(response.text, 'html.parser')
        try:
            allPages = soup.select('ul.pagination > li')[-2].text if soup.select('ul.pagination > li')[-2].text else None  # 翻页
            if allPages:
                for i in range(int(allPages)):
                    url = response.url + '/?page=' + str(i + 1)
                    yield scrapy.Request(url, meta=response.meta, callback=self.parse_essay)
        except:
