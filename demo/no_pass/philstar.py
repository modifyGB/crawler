...
class philstarSpider(scrapy.Spider):
    ...
    def parse_news_list(self, response):
        ...
        if soup.find_all("div", class_="tiles late"):
            temp_list = soup.find_all("div", meta=response.meta, class_="tiles late")
            ...
            for temp in temp_list[0:5]:
                response.meta["abstract"] = re.split("\r\n      |\r\n   ", temp.select_one("div.microsite_article_summary").text)[1] if temp.select_one("div.microsite_article_summary")else None
                if temp.select_one("div.microsite_article_title a"):
                    url = temp.select_one("div.microsite_article_title a").get("href")
                    url_list.append(url)
                    yield scrapy.Request(url, meta=response.meta, callback=self.parse_news)
        if url_list and re.findall(r"/\d+/\d+/\d+/\d+/", url_list[-1]):
